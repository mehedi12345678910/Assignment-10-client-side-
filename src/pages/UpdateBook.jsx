...
    if (!user) {
      setError("Authentication required to edit books. Redirecting to login...");
      setLoading(false);
      navigate("/login"); 
      return;
    }
...
    
        if (typeof fetchedBook.rating === 'string') {
            fetchedBook.rating = Number(fetchedBook.rating);
        }
...
        setIsSubmitting(false);
        setError(`Failed to update book: ${err.response?.data?.message || "Server error. Check console for details."}`);
      });
  };
