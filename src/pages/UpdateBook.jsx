...
const UpdateBook = () => {
	...
	useEffect(() => {
		...
		getBookById(id)
			...
			.catch(err => {
				...
				setLoading(false);
				setError("Failed to load book details for editing. It may have been deleted.");
			});
	}, [id, user, navigate]);
	...
	const handleSubmit = (e) => {
		...
		updateBook(id, book)
			.then(() => {
				setIsSubmitting(false);
				setSuccessMessage(`Book "${book.title}" updated successfully! Redirecting to My Books...`);
	...
	if (error || !book) {
		return (
			<div className="min-h-screen flex flex-col items-center justify-center bg-base-200 p-8">
				...
				<button onClick={() => navigate("/my-books")} className="btn btn-primary mt-6">
					Back to My Books
				</button>
			</div>
		);
			...
			<div className="max-w-3xl mx-auto card bg-base-200 shadow-2xl p-6 md:p-10">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="form-control pt-4">
						<button 
							type="submit" 
							className={`btn btn-warning w-full shadow-lg ${isSubmitting ? 'loading' : ''}`}
